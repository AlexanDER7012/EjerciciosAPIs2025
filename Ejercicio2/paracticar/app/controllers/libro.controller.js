...
const Libro = db.libros;  //Viene del Index
...
exports.librosDisponibles = (req, res)=>{
const disponible = req.query.disponible;

const condition = disponible ? {disponible:{ [Op.eq]: true}} : null;
Libro.findAll({where: condition})
.then(resultado=>{
    res.send(resultado)})
    .catch(error => {
        res.status(400).send({message:
            error.message || "Error al mostrar todos"
